-- Migration: Add VES Layers FIFO System for Historical Cost Tracking (FIXED VERSION)
-- Date: 2025-10-10 (CORRECTED)
...
--
-- FIXES:
-- 1. user_id changed from VARCHAR(64) to UUID
-- 2. Added automatic trigger to create/consume VES layers
-- 3. Added migration for existing VES balances (if any)

-- ============================================================================
-- 1. CREATE VES LAYERS TABLE (FIFO Tracking) - FIXED
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.bank_account_ves_layers (
    ...
    user_id UUID,  -- ✅ FIX 1: Changed from VARCHAR(64) to UUID
    ...
    CONSTRAINT fk_ves_layer_user
        FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE,  -- ✅ References auth.users
...
-- ============================================================================
-- 10. ✅ FIX 2: CREATE AUTOMATIC VES LAYERS MANAGEMENT TRIGGER
-- ============================================================================
...
DECLARE
    ...
    tx_exchange_rate DECIMAL(10,6);
    layer_id VARCHAR(64);
    ves_to_consume DECIMAL(18,2);
    remaining_to_consume DECIMAL(18,2);
    layer_rec RECORD;
    consume_from_layer DECIMAL(18,2);
BEGIN
    -- Only process for INSERT operations (when transaction is created)
    IF TG_OP != 'INSERT' THEN
        RETURN NEW;
    END IF;

    -- Only process if transaction has a bank account
    IF NEW.bank_account_id IS NULL THEN
        RETURN NEW;
    END IF;

    -- Get account currency and user_id
    SELECT currency, user_id INTO account_currency, account_user_id
    FROM public.bank_accounts
    WHERE id = NEW.bank_account_id;

    -- Only process VES accounts
    IF account_currency != 'VES' THEN
        RETURN NEW;
    END IF;

    -- Get exchange rate from transaction or use a default
    tx_exchange_rate := COALESCE(
        (SELECT rate FROM public.exchange_rates WHERE id = NEW.exchange_rate_id),
        50.0  -- Default fallback rate
    );

    -- Determine if this is an INFLOW or OUTFLOW transaction
    CASE NEW.type
        -- INFLOW: Create a new VES layer
        WHEN 'sale', 'payment', 'cash', 'ingreso' THEN
            -- Generate layer ID
            layer_id := gen_random_uuid()::text;

            -- Create new VES layer
            INSERT INTO public.bank_account_ves_layers (
                ...
                NEW.id,
                NEW.amount,
                NEW.amount,  -- Initially, all VES is remaining
                tx_exchange_rate,
                NEW.amount / tx_exchange_rate,
                account_user_id
            );

        -- OUTFLOW: Consume VES layers in FIFO order
        WHEN 'purchase', 'expense' THEN
            ves_to_consume := NEW.amount;
            remaining_to_consume := ves_to_consume;

            -- Loop through layers in FIFO order (oldest first)
            FOR layer_rec IN
            ...
            LOOP
                ...
                consume_from_layer := LEAST(layer_rec.remaining_ves, remaining_to_consume);

                -- Update the layer
                UPDATE public.bank_account_ves_layers
                SET remaining_ves = remaining_ves - consume_from_layer,
                    updated_at = NOW()
                WHERE id = layer_rec.id;

                remaining_to_consume := remaining_to_consume - consume_from_layer;
            END LOOP;

            -- Check if we consumed all required VES
            IF remaining_to_consume > 0 THEN
                RAISE WARNING 'Insufficient VES in layers for transaction %. Short by: %',
                    NEW.id, remaining_to_consume;
            END IF;

        ELSE
            -- Other transaction types don't affect VES layers
            NULL;
    END CASE;
...
-- Create trigger for automatic VES layer management
DROP TRIGGER IF EXISTS trigger_manage_ves_layers ON public.transactions;
...
-- ============================================================================
-- 11. ✅ FIX 3: MIGRATE EXISTING VES BALANCES (if any)
-- ============================================================================

-- Create initial layers for existing VES accounts with non-zero balances
DO $$
DECLARE
    latest_rate DECIMAL(10,6);
    account_rec RECORD;
BEGIN
    ...
    IF latest_rate IS NULL THEN
        latest_rate := 50.0;  -- Default fallback
    END IF;

    -- Create initial layer for each VES account with balance > 0
    FOR account_rec IN
    ...
    LOOP
        ...
        INSERT INTO public.bank_account_ves_layers (
            ...
            account_rec.id,
            NULL,  -- No transaction origin (initial balance)
            account_rec.amount,
            account_rec.amount,
            latest_rate,
            account_rec.amount / latest_rate,
            account_rec.user_id
        );

        -- Update historical_cost_usd
        UPDATE public.bank_accounts
        SET historical_cost_usd = account_rec.amount / latest_rate
        WHERE id = account_rec.id;
    END LOOP;
END $$;

-- ============================================================================
-- 12. CREATE VIEW FOR EASY NET WORTH CALCULATION
-- ============================================================================
...
-- Summary of changes:
-- ✅ Fixed user_id type to UUID
-- ✅ Created automatic VES layer management trigger
-- ✅ Migrated existing VES balances to initial layers
