...
import { createReceivable, updateReceivable, type Receivable } from "@/integrations/supabase/receivableService";
...
interface ReceivableFormModalOptimizedProps {
  isOpen: boolean;
  onClose: () => void;
...
export const ReceivableFormModalOptimized: React.FC<ReceivableFormModalOptimizedProps> = ({ 
  isOpen, 
  onClose, 
  receivable,
  clients,
  ...
  // Resetear formulario cuando cambia el receivable o se abre el modal
  useEffect(() => {
    if (isOpen) {
      if (receivable) {
        console.log('Loading receivable data:', receivable);
        setClientId(receivable.client_id || '');
        setAmount(receivable.amount?.toString() || '');
        setCurrency(receivable.currency || 'USD');
        setReference(receivable.description || '');
        setDueDate(receivable.due_date ? new Date(receivable.due_date) : new Date());
        setStatus((receivable.status as 'pending' | 'overdue' | 'paid') || 'pending');
        setNotes(receivable.notes || '');
        setInterestRate(receivable.interest_rate?.toString() || '');
        setInstallments(receivable.installments?.toString() || '1');
        setPaymentMethod(receivable.payment_method || 'transfer');
        setBankAccountId(receivable.bank_account_id || '');
        setCommission(receivable.commission?.toString() || '0');
      } else {
        console.log('No receivable data, resetting form');
        resetForm();
      }
    }
  }, [isOpen, receivable]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing && receivable) {
        await updateReceivable(receivable.id, receivableData);
