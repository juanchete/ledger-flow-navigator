-- SCRIPT COMPLETO CORREGIDO PARA CREAR SISTEMA DE CAPAS VES FIFO
...
-- ============================================================================
-- 2b. ASEGURAR QUE LA TABLA VES_LAYERS TIENE TODOS LOS CAMPOS
-- ============================================================================

-- Agregar is_active si no existe
ALTER TABLE public.bank_account_ves_layers
ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;

-- Asegurar que registros existentes tienen is_active = true
...
DECLARE
  ...
  new_layer_id TEXT;
  rate DECIMAL(10,6);
  trans_exchange_rate DECIMAL(10,6);
BEGIN
  ...
  FOR trans IN
    ...
    WHERE ba.currency = 'VES'
      ...
      AND t.amount > 0
      AND t.type IN ('sale', 'cash', 'balance-change', 'payment')
  ...
  LOOP
    ...
    -- Obtener tasa de cambio de la tabla exchange_rates si existe
    IF trans.exchange_rate_id IS NOT NULL THEN
      SELECT rate INTO trans_exchange_rate
      FROM public.exchange_rates
      WHERE id = trans.exchange_rate_id;
    END IF;

    -- Determinar tasa de cambio (prioridad: transaction rate > default 200)
    rate := COALESCE(trans_exchange_rate, 200.0);
